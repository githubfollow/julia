...
function narrow_yakc!(stmt, calltype)
    if isa(calltype, PartialYAKC)
        # Narrow yakc type
        if isa(calltype.ci, OptimizationState)
            stmt.args[3] = stmt.args[4] = widenconst(calltype.ci.src.rettype)
        elseif isa(calltype.ci, Method)
            m = calltype.ci
            stmt.args[6] = m
            stmt.args[3] = stmt.args[4] = widenconst(m.unspecialized.cache.rettype)
        end
    end
end
...
function process_simple!(ir::IRCode, todo::Vector{Pair{Int, Any}}, idx::Int, state::InliningState)
    ...
    elseif stmt.head === :new_yakc
        narrow_yakc!(stmt, ir.stmts[idx][:type])
        return nothing
    end
    ...
    if sig.ft ⊑ Core.YAKC
        callee = stmt.args[1]
        if isa(callee, SSAValue) && isexpr(ir.stmts[callee.id][:inst], :new_yakc) && length(ir.stmts[callee.id][:inst].args) == 7
            ir′ = ir.yakcs[ir.stmts[callee.id][:inst].args[end]::Int]::IRCode
